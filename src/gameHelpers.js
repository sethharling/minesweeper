...
export const createStage = () => {
	...
	for (let i = 0; i < rows.length; i++) {
		for (let j = 0; j < rows[i].length; j++) {
			if (rows.indexOf(i) === 0) { // if its the top row
				if (rows.indexOf(j) === 0) { //if its the top left
				...
				} else if (rows.indexOf(j) === STAGE_WIDTH - 1) { // top right
					//check west
					//check southwest
					//check south
				} else { // other blocks in top row
					//check west
					//check southwest
					//check south
					//check southeast
			...
			} else if (rows.indexOf(i) === STAGE_HEIGHT - 1) { // bottom row
				if (rows.indexOf(j) === 0) { // bottom left

				} else if (rows.indexOf(j) === STAGE_WIDTH - 1) { // bottom right

				} else { // other blocks in bottom row

				}

			} else { // the rest of the blocks 
